		...
		if event == "start":
			...
			#	words = []
				for word_element in elem.iter("{kk-ocr}String"):
					...
					#words.append(word)
					ocr_images.append([word, sub_image])
				#ocr_images.append([" ".join(words), images])
	return ocr_images

def extract(n_images, data_type):
	extracted = 0
	for alto_file in tqdm.tqdm(os.listdir(config.altos + "/" + data_type), desc="Extracting OCR sub-images..."):
		...
		for ocr_image_i, ocr_image_data in enumerate(ocr_images):
			ocr_word, ocr_word_image = ocr_image_data
			#for ocr_word_image_i, ocr_word_image in enumerate(ocr_word_images):
			ocr_word_image.save(config.ocr_images + "/" + data_type + "/" + alto_file.split(".xml")[0] + "_" + str(ocr_image_i) + ".png")
			open(config.ocr_texts + "/" + data_type + "/" + alto_file.split(".xml")[0] + "_" + str(ocr_image_i) + ".txt", "w").write(ocr_word)
			extracted += 1
			if extracted >= n_images:
				return
...
if __name__ == "__main__":
	...
	for data_type in ["train", "test"]:
		logger.info("Extracting first {} OCR sub-images from {} set".format(n_images, data_type))
		extract(n_images, data_type)
